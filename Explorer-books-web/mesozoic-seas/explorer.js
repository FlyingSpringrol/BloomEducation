(function () {
  ...
  function openStory(key) { var item = entries[key]; if (!item) return; var title = clean(item.title); document.getElementById('story-title').textContent = title; document.getElementById('story-kicker').textContent = item.size || 'Mesozoic marine life'; var paragraphs = item.paragraphs || (item.text && item.text !== 'Text' ? [item.text] : []); document.getElementById('story-body').textContent = paragraphs.join('\n\n') || 'Select this animal in the reconstructed scene to identify it and compare its place in the ancient marine ecosystem.'; var image = document.getElementById('story-image'); var visual = image.closest('.story-visual'); visual.classList.remove('is-ready', 'is-loading'); image.alt = title + ' reconstruction'; image.hidden = true; image.removeAttribute('src'); var imageRequest = String((Number(image.dataset.request) || 0) + 1); image.dataset.request = imageRequest; if (item.image) { visual.classList.add('is-loading'); var imageLoader = new Image(); imageLoader.onload = function () { if (image.dataset.request === imageRequest) { image.src = item.image; image.hidden = false; requestAnimationFrame(function () { visual.classList.remove('is-loading'); visual.classList.add('is-ready'); }); } }; imageLoader.onerror = function () { if (image.dataset.request === imageRequest) visual.classList.remove('is-loading'); }; imageLoader.src = item.image; } modal.hidden = false; document.body.classList.add('modal-open'); modal.querySelector('.story-close').focus(); }
